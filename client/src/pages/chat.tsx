                        ...
                        <div className="mt-3">
                          {(result.nft_tx_hash || nftMintResult) ? (
                            <a
                              ...
                              target="_blank"
                              rel="noopener noreferrer"
                              className="flex items-center gap-2 px-4 py-3 rounded-lg border border-green-500/30 bg-green-500/5 hover:bg-green-500/10 transition-colors group"
                              ...
                              <div className="min-w-0 flex-1">
                                <div className="text-xs font-mono font-bold text-green-600 dark:text-green-300">
                                  ...
                                  {(nftMintResult?.nft_token_id || result.nft_token_id) && (
                                    <span className="text-muted-foreground font-normal ml-2">#{nftMintResult?.nft_token_id || result.nft_token_id}</span>
                                  )}
                                </div>
                                <div className="text-[10px] font-mono text-muted-foreground truncate">{nftMintResult?.nft_tx_hash || result.nft_tx_hash}</div>
                              </div>
                              <ExternalLink size={14} className="text-muted-foreground group-hover:text-green-400 transition-colors flex-shrink-0" />
                            </a>
                          ) : result.nft_mint_available ? (
                            <button
                              onClick={mintNft}
                              disabled={nftMinting}
                              className="w-full flex items-center gap-2 px-4 py-3 rounded-lg border border-green-500/30 bg-green-500/5 hover:bg-green-500/10 transition-colors group disabled:opacity-50 disabled:cursor-wait"
                            ...
                            <div className="flex items-center gap-2 px-4 py-3 rounded-lg border border-border bg-muted/30">
                              <ShieldCheck size={16} className="text-muted-foreground" />
                              <div className="min-w-0 flex-1">
                                <div className="text-xs font-mono font-bold text-muted-foreground">ProofOfLogic NFT</div>
                                <div className="text-[10px] font-mono text-muted-foreground/60">{result.nft_error}</div>
                              </div>
                            </div>
                          ) : null}
                        </div>
