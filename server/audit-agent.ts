...
When the request includes a "trade_params" object (protocol, pair, direction, leverage, notional_usd, stop_loss_pct, take_profit_pct, entry_price, timeframe), you MUST cross-reference these hard numbers against the narrative in "strategy_memo". Flag any mismatch between the stated reasoning and the actual trade parameters — e.g. memo claims breakout but entry price is below the stated level, or leverage is extreme relative to stated risk tolerance. Missing stop-loss or take-profit in high-leverage trades should trigger DJZS-X01 (UNHEDGED_EXECUTION).
  ...
  const response = await fetch(`${VENICE_API_BASE}/chat/completions`, {
    ...
    body: JSON.stringify({
      ...
      messages: [
        ...
        {
          role: "user",
          content: `${request.intelligence_context ? request.intelligence_context + '\n\n' : ''}STRATEGY MEMO FOR AUDIT:\n\n${request.strategy_memo}${request.trade_params ? `\n\nSTRUCTURED TRADE PARAMETERS:\n${JSON.stringify(request.trade_params, null, 2)}` : ''}\n\n---\nPerform a full logic audit. Score the risk. Identify bias. Find the flaws. Be adversarial.${request.intelligence_context ? ' Factor the Founder Intelligence Brief into your analysis — use the historical bias patterns, narrative drift signals, and emotional spike data to inform your verdict.' : ''}${request.trade_params ? ' Cross-reference the trade_params against the strategy_memo — flag any parameter/narrative mismatches.' : ''}`,
        },
      ],
